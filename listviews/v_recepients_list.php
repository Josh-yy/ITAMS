	...
	<div class="card-header">
		<div class="input-group " >
	 	<?php
	 		if($page>1){
	 		?>
	 		<button class="btn btn-secondary" onclick="listrecord('listviews/v_recepients_list','display_list','<?php echo ($page-(1))  + '&search=' ?>')"><i class="ti ti-arrow-left"></i> Prev</button>
	 		<?php
	 		}
	 	?>
		<button class="btn btn-light-secondary">Showing Page <?php echo $page ?> of <?php echo $number_of_page ?></button>
	  	<?php
	 		...
	 		?>
	 		<button class="btn btn-secondary" onclick="listrecord('listviews/v_recepients_list','display_list','<?php echo ($page + 1) ?>' + '&search=' )">Next <i class="ti ti-arrow-right"></i></button>
	 		<?php
	 		}
	 	?>
</div>
		...
		<div class="card-body">
			...
			<thead>
				...
				foreach ($data as $row) {
					...
					if(get_exist2("select * from t_asset_disposal_recepients where user_id = '".$row['id']."'",$db)==0){
			...
			?>	
				...
				<tr role="button" > 
					<td><input type="checkbox" name="lab_id[]" value="<?php echo $row['id'] ?>"></td>
