	...
	$final_sql = "";
		if(isset($_REQUEST['search'])){
			if($_REQUEST['search']!==""){
				...
				$final_sql = "select *  from m_hardware_assets where status != 'Disposed' LIMIT " . $page_first_result . ',' . $results_per_page; 
			}
		}else{
				$final_sql = "select *  from m_hardware_assets where status != 'Disposed' LIMIT " . $page_first_result . ',' . $results_per_page; 
			}
	...
	<div class="card-header">
		<div class="input-group " >
	 	<?php
	 		if($page>1){
	 		?>
	 		<button class="btn btn-secondary" onclick="listrecord('listviews/v_hardware_info_list','display_hardware_assets','<?php echo ($page-(1))  ?>' + '&search')"><i class="ti ti-arrow-left"></i> Prev</button>
	 		<?php
	 		}
	 	?>
		<button class="btn btn-light-secondary">Showing Page <?php echo $page ?> of <?php echo $number_of_page ?></button>
	  	<?php
	 		...
	 		?>
	 		<button class="btn btn-secondary" onclick="listrecord('listviews/v_hardware_info_list','display_hardware_assets','<?php echo ($page+1) ?>' + '&search=' )">Next <i class="ti ti-arrow-right"></i></button>
	 		<?php
	 		}
	 	?>
</div>
...
function close_modal(id,modal_id,c,locator){
	...
	listrecord('listviews/asset_information_viewer','asset_other_information',id);
