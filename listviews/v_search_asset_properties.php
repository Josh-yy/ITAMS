...
require("../assets/settings/functions.php");
	...
	$final_sql = "";
		if(isset($_REQUEST['search'])){
			if($_REQUEST['search']!==""){
				$final_sql = "select *  from m_hardware_assets a where code  LIKE '%".$_REQUEST['search']."%' OR a.machine_name LIKE '%".$_REQUEST['search']."%' OR a.serial_number LIKE '%".$_REQUEST['search']."%' ";
		}
		else{
	...
	<div class="card-header">
		<div class="input-group " >
	 	<?php
	 		if($page>1){
	 		?>
	 		<button class="btn btn-secondary" onclick="listrecord('listviews/v_search_asset_properties','display_hardware_assets','<?php echo ($page-(1))  ?>' + '&search')"><i class="ti ti-arrow-left"></i> Prev</button>
	 		<?php
	 		}
	 	?>
		<button class="btn btn-light-secondary">Showing Page <?php echo $page ?> of <?php echo $number_of_page ?></button>
	  	<?php
	 		...
	 		?>
	 		<button class="btn btn-secondary" onclick="listrecord('listviews/v_search_asset_properties','display_hardware_assets','<?php echo $page+1 ?>' + '&search=' )">Next <i class="ti ti-arrow-right"></i></button>
	 		<?php
	 		}
	 	?>
</div>
		...
		<div class="card-body table-responsive">
			...
			<?php
				foreach ($data as $row) {	
			?>
				<tr role="button" onclick="close_modal('<?php echo $row['auto_qr_code'] ?>','mdl_search_list','<?php echo $row['serial_number'] ?>')">
...
<script>
function close_modal(id,modal_id,c){
	loadcontent('search_hardware_asset','asset_information',id);
	$('#txtassetname').html("Serial # : " + c);
	listrecord('listviews/asset_properties','asset_other_information',id);
