	...
	<div class="card-header">
		<div class="input-group " >
	 	<?php
	 		if($page>1){
	 		?>
	 		<button class="btn btn-secondary" onclick="listrecord('listviews/v_assets_info','display_list','<?php echo ($page-(1))  ?>' + '&search')"><i class="ti ti-arrow-left"></i> Prev</button>
	 		<?php
	 		}
	 	?>
		<button class="btn btn-light-secondary">Showing Page <?php echo $page ?> of <?php echo $number_of_page ?></button>
	  	<?php
	 		...
	 		?>
	 		<button class="btn btn-secondary" onclick="listrecord('listviews/v_assets_info','display_list','<?php echo ($page+1) ?>' + '&search=' )">Next <i class="ti ti-arrow-right"></i></button>
	 		<?php
	 		}
	 	?>
</div>
				...
				<td>
					<div role="group" class="btn-group-sm btn-group btn-group-toggle">
						...
						<a href="setup_software_installed?hid=<?php echo $row['id'] ?>" class="btn btn-primary" title="Manage Installed Software"><i class="ti ti-settings"></i></a>
							...
							<button  class="btn btn-danger btn-sm" style="display:none" id="btn_delete_user" onclick="delete_record('<?php echo $row['id'] ?>','<?php echo $row['machine_name'] ?>','deleteassetinfo','../listviews/v_assets_info','display_list',1)"><i class="ti ti-trash"></i></button>
					</div>
				</td>
				</tr>
			<?php
