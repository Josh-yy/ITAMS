			...
			?>
				...
				<td>
					<div role="group" class="btn-group-sm btn-group btn-group-toggle">
						<a href="manage_class?class_id=<?php echo $row['id'] ?>" class="btn btn-primary"><i class="ti ti-folders"></i> Manage Class</a>
							<button class="btn-pill btn btn-warning" onclick="get_edit('<?php echo $row['id'] ?>','<?php echo $row['code'] ?>','<?php echo $row['class_name'] ?>','<?php echo $row['sy_id'] ?>','<?php echo $row['adviser_id'] ?>')" data-bs-toggle="modal" data-bs-target="#mdl_update"><i class="ti ti-edit"></i></button>
							<button  class="btn btn-danger btn-sm" id="btn_delete_user" onclick="delete_record('<?php echo $row['id'] ?>','<?php echo $row['class_name'] ?>','deleteclass','../listviews/v_classes','display_list',1)"><i class="ti ti-trash"></i></button>
					</div>
				</td>
				</tr>
			<?php
