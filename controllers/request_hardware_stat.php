...
$status = $_POST['status'];
$transaction_code = get_column2("transaction_code","select * from m_hardware_assets where id = '$asset_id'",$db);

		$db->query("insert into t_hardware_update_status values (id, '$asset_id','$status','$reasons','$newdate',0,null,null)");
	//	$db->query("UPDATE m_hardware_assets set status = 'Non-Functonal' where id = '".$asset_id."'");
		//save notification
       // $db->query("INSERT INTO t_asset_disposal VALUES ()");
       	$db->query("INSERT INTO t_asset_notification VALUES (id,'$transaction_code',0,(select approver_id from m_approver),'$newdate','status-update')");
        echo 0;
...
if(get_column2("is_enabled","select  * from t_email_facility_settings",$db)==1){
    ...
    activity_log_maker("Status Update",$activity,$_SESSION['data']['account_id'],$db);
	...
	foreach($data as $row){
		...
		$sender_email = get_column2("email_address","select  * from t_email_facility_settings",$db);
			...
			//send_email($serial,"Asset Non-Functional Update Notification",$email,$username,$api_key,$sender_email,$reasons,$non_functional_date);
	}
            ...
            <div class="col-md-8">
                ...
                <div class="card custom-card">
                    ...
                    <div class="card-body custom-card-body">
                     ...
                     This email informs you that the asset with a serial number <b>'.$serial.'</b> is for non-functional tagging due to <b>'.$reason.'</b> effective <b>' .$edate.'</b>.
                     In this regard we are sending this email for you to be notified of its replacement as soon as possible. pleas eaccess the system to approve the request.
