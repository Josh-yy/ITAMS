...
$qr_code = $db->sanitize(encrypt($transaction_code));
	if(get_exist2("select * from m_hardware_assets where code='$asset_code' or serial_number = '$serialno' ",$db)>0){
        ...
        $sl = "";
        if(get_exists2("select * from v_temp_image",$db)==0){
            $sql  ="insert into m_hardware_assets values (id, '$transaction_code','$asset_code','$machinename','$modelno','$serialno','$status','$qr_code',null,'$assetcat',1,'$manufacturer_name','$installable','$datepurchase','$newdate','".$_SESSION['data']['account_id']."')";
        }else{
            $sql  ="insert into m_hardware_assets values (id, '$transaction_code','$asset_code','$machinename','$modelno','$serialno','$status','$qr_code',(select filename from v_temp_image),'$assetcat',1,'$manufacturer_name','$installable','$datepurchase','$newdate','".$_SESSION['data']['account_id']."')";
        }
