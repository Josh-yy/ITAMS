          ...
          <div class="card my-5">
            <div class="card-body">
              ...
              <div class="row">
                <div class="d-flex justify-content-center">
                  <div class="auth-header">
                    ...
                    <p class="f-16 mt-2">Forgot password wizard</p>
              ...
              <div class="form-floating mb-3">
                <input type="text" class="form-control" id="txtusername" name="email" placeholder="Email address / Username" />
              ...
              <div class=" mt-4">
                <a href="login" class="btn btn-outline-secondary"><i class="material-icons-two-tone">check</i>Sign-in Account</a>
                <button type="submit" class="btn btn-secondary" id="btnreset"><i class="material-icons-two-tone">check</i> Reset Password</button>
              </div>
             </form>
            </div>
          </div>
...
$('#frmreset').on("submit", function(e){
  e.preventDefault();
  if($('#txtusername').val()==""){
    fire_message("Notifier","Enter your email address","info");
    $('#txtusername').focus();
  }else{
    $.ajax({
      type:'POST',
      url:'controllers/verify_account',
      cache:false,
      ...
      success:function(data){
        if(data==0){
          fire_message("Notifier","The email does not exist!","error");
        }else{
           $.ajax({
              type:'POST',
              url:'controllers/forgot_password',
              cache:false,
              data : 'email=' + $('#txtusername').val(),
              beforeSend:function(){
                $('#btnreset').attr('disabled',true);
             ...
             $('#btnreset').attr('disabled',false);
                if(data==0){
                 fire_message("Notifier","Check your email! we have successfully reset your account password!","success");
                }
              }
            })
        }
