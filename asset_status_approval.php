                                ...
                                ?>
                                  <tr>
                                    ...
                                    <td> 
                                       ...
                                       <button id="btnReject<?php echo $row['id'] ?>" onclick="reject_selection('<?php echo $row['id'] ?>','<?php echo get_column2("serial_number","select * from m_hardware_assets where id = '".$row['asset_id']."'",$db) ?>','reject','','','')" class="btn btn-outline-secondary btn-sm"><i class="ti ti-close"></i> Reject</button>
...
?>
<script>
 function reject_selection(id, name, action, url, display_area, param) {
    try {
        Swal.fire({
            title: 'Reject selected item?',
            text: "Are you sure you want to reject asset " + name + '?',
            icon: 'question',
            ...
            if (result.isConfirmed) {
                $.ajax({
                    type: 'POST',
                    url: 'controllers/reject_hardware_status.php',
                    cache: false,
                    data: 'action=' + action + "&id=" + id,
                    beforeSend: function () {
                        $('#btnReject' + id).attr('disabled', true);
                    },
        ...
        console.error('Error in reject_selection:', error);
    }
}
