...
$asset_id = $_REQUEST['hadware_asset_id'];
	...
	$sql = "select a.*, b.typename from m_users a inner join m_usertype b on a.usertype = b.id WHERE  a.usertype <> 2";
	$number_of_result = get_exist2($sql,$db);
	$number_of_page = ceil ($number_of_result / $results_per_page); 
	$final_sql = "";
		if(isset($_REQUEST['search'])){
			if($_REQUEST['search']!==""){
				$final_sql = "select a.*, b.typename from m_users a inner join m_usertype b on a.usertype = b.id WHERE  a.usertype <> 2 and  username LIKE '%".$_REQUEST['search']."%' and is_active = 1 ";
		}
		else{
				$final_sql = "select a.*, b.typename from m_users a inner join m_usertype b on a.usertype = b.id WHERE a.usertype <> 2 and is_active = 1 LIMIT " . $page_first_result . ',' . $results_per_page; 
			}
		}else{
				$final_sql = "select a.*, b.typename from m_users a inner join m_usertype b on a.usertype = b.id WHERE a.usertype <> 2 and is_active = 1 "  . $page_first_result . ',' . $results_per_page; 
			}
