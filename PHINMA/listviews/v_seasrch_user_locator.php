...
$asset_id = $_REQUEST['hadware_asset_id'];
	...
	$final_sql = "";
		if(isset($_REQUEST['search'])){
			if($_REQUEST['search']!==""){
				$final_sql = "select a.*, b.typename from m_users a inner join m_usertype b on a.usertype = b.id WHERE  a.usertype <> 2 and  username LIKE '%".$_REQUEST['search']."%' ";
		}
		else{
				$final_sql = "select a.*, b.typename from m_users a inner join m_usertype b on a.usertype = b.id WHERE a.usertype <> 2 LIMIT " . $page_first_result . ',' . $results_per_page; 
			}
		}else{
				$final_sql = "select a.*, b.typename from m_users a inner join m_usertype b on a.usertype = b.id WHERE a.usertype <> 2" . $page_first_result . ',' . $results_per_page; 
			}
